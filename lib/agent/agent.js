...
var Agent = function(conf) {
  ...
  this.reconnecting = false;
	...
	this.limit = 1;
	this.countimes = this._count/this.limit;
	this.times = 1;
	this.machines = [];
};
...
Agent.prototype = {
  ...
  run:function(){
  	var agent = this;
  	agent.runusers = agent._conf.app.data.slice(0,this.times * this.limit);
  	this._count = __.size(agent.runusers);
  	agent._log.info('run with ' + __(agent.runusers).size() + ' robots  ');
  	monitor.clear();
  	var machines = [];
  ...
  close:function(){
  	...
  	agent.times++;
  	console.log('agent.times' + agent.times + ' ' + agent.countimes);
  	if (agent.countimes>=agent.times){
  		agent.run();
  	}
  },
  ...
  check: function() {
    ...
    if (this._count==0){
    	...
    	var d = monitor.getData();
			agent.socket.emit('report',{id:agent.times,data:d});
			agent.close();
