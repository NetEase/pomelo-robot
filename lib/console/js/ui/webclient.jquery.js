...
function jsonToTable(json) {
    ...
    for ( var i in json) {
      var ele = json[i];
      txt += "<tr><td class=label>" + ele.key + "</td><td>" + ele.max + "</td><td>" + ele.min + "</td><td>" + Math.round(ele.avg) + "</td><td>" + Math.round(ele.raw) + "</td><td>" + Math.round(ele.qps) + "</td><td>" + Math.round(ele.tps) + "</td><td>" + ele.num + "</td></tr>";
    }
    return txt;
};
...
var updateTimesData = function(agent,user,stimeData){
    ...
    for (var key in act) {
        for (var agent  in stimeData) {
          ...
          if (!exist){
            summary[key] = {key: key, max: single.max, min: single.min, avg: single.avg, raw: single.raw, num: single.num};
  ...
  for (var index in sortSum) {
    ...
    var key = ex.key;
    var columns = [key, 'max', 'min', 'avg', 'raw', 'qps', 'tps'];
    var grow = gdata[key] || [];
    var last = grow[grow.length-1];
    if (!last || last[0]!==ex.num) {
      grow.push([ex.num, ex.max, ex.min, ex.avg, ex.raw, ex.qps, ex.tps]);
    } 
    gdata[key] = grow;
    var qpschart = {};
