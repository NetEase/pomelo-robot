...
function jsonToTable(json) {
    ...
    for ( var i in json) {
      var ele = json[i];
      txt += "<tr><td class=label>" + ele.key + "</td><td>" + ele.max + "</td><td>" + ele.min + "</td><td>" + Math.round(ele.avg) + "</td><td>" + Math.round(ele.qps) + "</td><td>" + ele.num + "</td><td>" + Math.round(ele.tps) + "</td><td>" + Math.round(ele.raw) + "</td></tr>";
    }
    return txt;
};
...
var updateTimesData = function(agent,user,stimeData){
    // console.log('stimeData = ', JSON.stringify(stimeData));
    var conns = agent;
    var act = {};
    var summary = {};
    ...
    for (var key in act) {
        for (var agent  in stimeData) {
          ...
          if (!exist){
            summary[key] = {key: key, min: single.min, max: single.max, avg: single.avg, num: single.num, tps: single.tps, raw: single.raw};
            summary[key].qps = 1000/single.avg*conns;
            summary[key].tps = single.tps * conns;
          } else {
  ...
  for (var index in sortSum) {
    ...
    var key = ex.key;
    var columns = [key, 'avg', 'min', 'max', 'qps', 'tps', 'raw'];
    var grow = gdata[key] || [];
    var last = grow[grow.length-1];
    if (!last || last[0]!==ex.num) {
      grow.push([ex.num, ex.avg, ex.min, ex.max, ex.qps, ex.tps, ex.raw]);
    } 
    gdata[key] = grow;
    var qpschart = {};
