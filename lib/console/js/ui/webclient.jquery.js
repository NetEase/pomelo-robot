...
function jsonToTable(json) {
    ...
    for ( var i in json) {
      var ele = json[i];
      txt += "<tr><td class=label>" + ele.key + "</td><td>" + ele.max + "</td><td>" + ele.min + "</td><td>" + Math.round(ele.avg) + "</td><td>" + Math.round(ele.qps) + "</td><td>" + ele.num + "</td><td>" + Math.round(ele.tps) + "</td></tr>";
    }
    return txt;
};
...
var updateTimesData = function(agent,user,stimeData){
    ...
    for (var key in act) {
        for (var agent  in stimeData) {
          ...
          if (!exist){
            summary[key] = {key: key, min: single.min, max: single.max, avg: single.avg, num: single.num, tps: single.tps};
  ...
  for (var index in sortSum) {
    ...
    var key = ex.key;
    var columns = [key, 'avg', 'min', 'max', 'qps', 'tps'];
    var grow = gdata[key] || [];
    var last = grow[grow.length-1];
    if (!last || last[0]!==ex.num) {
      grow.push([ex.num, ex.avg, ex.min, ex.max, ex.qps, ex.tps]);
    } 
    gdata[key] = grow;
    var qpschart = {};
