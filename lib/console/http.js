...
HttpServer.prototype.serveFile_ = function(file, response) {
	    ...
	    __.each(pdata,function(val,key){
	    		__.each(val,function(kval,akey){
		    		var single = {};
		    		_show = true;
		    		single['name'] = key + akey;
		    		single['uid'] = key+ akey;
		    		console.log('uid ' + single['uid']);
	    			var loginTimes = kval;
		    		var rows = [];
		    		var i = 1;
		    		var sum = 0;
		    		...
		    		__.each(loginTimes,function(time){
		    			var row = [i++,time];
		    		...
		    		var avg = sum/i;
		    		single['summary'] = {'max':max,'min':min,'avg':avg,'rqs':i*1000/avg};
		    		single['charts'] =  {"latency":{"name":"latency","uid":single['uid'],"columns":["time",akey],"rows":rows}};
