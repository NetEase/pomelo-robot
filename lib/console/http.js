    	...
    	var single = {};
  		...
  		for (var i = 0;i< maxId;i++) {
  			...
  			__.each(keycolumns,function(dkey){
  				...
  				if (!!val[dkey][i]) {
  					rows.push(val[dkey][i]);
  				} else {
  					var lastKeyVal = glastkeyData[dkey];
  					if (!lastKeyVal) {
  						lastKeyVal = val[dkey][i-1];
  						glastkeyData[dkey] = lastKeyVal;
  					}
						rows.push(lastKeyVal);
  				}
  				_vaild = true;
	  		});	
  			grows.push(rows);
  		...
  		__.each(keycolumns,function(dkey){
  			...
  			var kdata = val[dkey];
				...
				summary = {'max':max,'min':min,'avg':avg,'rqs':Math.round(i*1000/avg)};
				gsummary[dkey] = JSON.stringify(summary);
  		});	
			single['summary'] = gsummary;
  		single['charts'] =  {"latency":{"name":"robot","uid":single['uid'],"columns":gcolumns,"rows":grows}};
  		if (_vaild)	mdata.push(single);
  });
  if (_show) {
