...
robot.on('connector.loginHandler.login', function(data){
    ...
    if (player.id <= 0) { 
        ...
        robot.interval(move,Math.round(Math.random()*5000));
    }
});
...
robot.on('onAttack',function(data){
    ...
    if (data.result.result === 2) {
        ...
        if (attackId === selfId || targetId === selfId) {
            ...
            var entity = pomelo.entitys[data.target];
            if (!!entity && entity.type ==='npc' ) {
                delete pomelo.entitys[data.target];
            }
        }
    }
    //killMap[data.attacker] = true;
    //killMap[data.target] = true;
});
...
var move = function(){
    ...
    var entity = nearEntity;
        ...
        //pomelo.isMove = true;
        robot.pushMessage({route:'area.playerHandler.pickItem', areaId:pomelo.areaId, playerId:pomelo.playerId, targetId:nearstId});
		}
...
robot.on('onPickItem', function(data){
    ...
    delete item;
    pomelo.isMove = false;
    console.log('pic %j',item);
    msgTempate.content = '捡到一个XXOO的'+ item.name+'玩意';
    robot.pushMessage(msgTempate);
});
