...
Starter.prototype.run = function(env,message,clients){
    if (!clients) {
        clients = ['127.0.0.1'];
        this.prepare(env,message,clients);
    } else {
        this.prepare(env,message,clients);
    }
};
...
Starter.prototype.prepare = function(env,message,clients){
    var self = this;
	  ...
	  var time = parseInt(message.time) || 1;
	  var ids = 0;
    var index = 1;
    for(var ipindex in clients) {
        for (var i = 0;i<count;i++) {
  	        ids +=limit;
		        cmd =  'cd '+ process.cwd()+ ' && node lib/main.js ' + env + ' client agent' + index  + ' ' + (limit) +' ' + (ids-limit) + ' ' + time;           var ip = clients[ipindex];
            if (ip==='127.0.0.1') {
                self.localrun(cmd);
            } else {
                self.sshrun(cmd,ip);
