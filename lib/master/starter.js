...
Starter.prototype.prepare = function(main,message,clients){
  console.error('0 ~ ' + Date() + ' ~ kill is called ... ', + childrenProc.length);
  childrenProc.forEach(function(child) {
    console.error('1 ~ ' + Date() + ' ~ kill is called ...');
    child.kill('SIGKILL');
  });
  childrenProc = [];
    var self = this;
...
function spawnProcess(command, options, callback) {
    var child = null;
    if (!!options[0]) {
      console.error('spawnProcess spawn is running...');
    	  child = cp.spawn(command, options);

    } else {
      console.error('spawnProcess exec is running...');
